...
function LearningTargetsInput() {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = course.learning_targets.findIndex((target: LearningTarget) => target.id === active.id);
      const newIndex = course.learning_targets.findIndex((target: LearningTarget) => target.id === over.id);
    ...
    <div className="space-y-3">
      ...
      {course.learning_targets.length === 0 ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={course.learning_targets.map((target: LearningTarget) => target.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {course.learning_targets.map((target: LearningTarget) => (
