...
function enhanceHTMLWithTailwind(html: string): string {
  ...
  $('code').each((_, el) => {
    if ($(el).parent().is('pre')) return; // skip block code
...
async function getCourseMaterialWithNavigation(
  courseId: string,
  sectionId: string,
  materialId: string
) {
...
export default async function MaterialDetailsPage({ params }: MaterialDetailsPageProps) {
  ...
  const {
    ...
    section,
    navigation,
  } = await getCourseMaterialWithNavigation(
    courseId, sectionId, materialId
  );
    ...
    <div className="container mx-auto px-6">
          ...
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <NextLink href="/a/course">Course</NextLink>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <NextLink href={`/a/course/${courseId}`}>{course.title}</NextLink>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
      ...
      <div className="flex justify-between items-center mt-12 pt-8 border-t border-gray-200">
        <div className="flex-1">
          {navigation.previous && (
            <NextLink
              href={`/a/course/${courseId}/${sectionId}/${navigation.previous.id}`}
        ...
        <div className="flex-1 text-right">
          {navigation.next && (
            <NextLink
              href={`/a/course/${courseId}/${sectionId}/${navigation.next.id}`}
              className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
            >
