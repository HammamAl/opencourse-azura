...
/**
 * Validates the server session to ensure the user is an authenticated admin.
 * @param session The NextAuth session object.
 * @returns An error response if validation fails, otherwise null.
 */
function validateAdminSession(session: Session | null): NextResponse | null {
  if (!session) {
...
/**
 * Validates the request headers and the file buffer against configured rules.
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const sessionError = validateAdminSession(session);
  if (sessionError) {
    return sessionError;
  }
  ...
  try {
    ...
    const validationResult = await validateFileUpload(request, buffer);
    if (validationResult instanceof NextResponse) {
      return validationResult;
    }

    const imageUrl = await uploadImageToAzure(buffer, validationResult);

    return NextResponse.json({ cover_image_url: imageUrl });
  } catch (error) {
