...
/**
 * Handles image upload to Azure Blob Storage via a backend API
...
export const handleImageUpload = async (file: File, onProgress?: (event: { progress: number }) => void, abortSignal?: AbortSignal): Promise<string> => {
  // --- Basic File Validation (optional, as backend also validates) ---
  if (!file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", "/api/tiptap/image", true);
